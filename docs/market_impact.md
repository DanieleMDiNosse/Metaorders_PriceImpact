...
This document describes in detail the procedure used in `scripts/metaorder_computation.py` to estimate power-law market impact functions from trade-by-trade data. The emphasis is on:
...
Source logs (one-dimensional fits + surfaces):

- Proprietary (all): `out_files/ftsemib/logs/pipeline_20260220_190521/metaorder_computation_member_prop_true_nat_all.log`
- Client / non-proprietary (all): `out_files/ftsemib/logs/pipeline_20260220_190521/metaorder_computation_member_prop_false_nat_all.log`
- Nationality slices: `..._nat_foreign.log`, `..._nat_it.log` in the same folder

### One-dimensional impact curve (log-binned WLS)

Impact curve fitted on metaorders after `Q/V > 1e-5` and `Participation Rate < 1.0`:

| Group | Counts (before Q/V → after Q/V → after PR<1.0) | Power-law fit $Y \phi^\gamma$ | $R^2_{\log}$ / $R^2_{lin}$ | Log overlay $a\log_{10}(1+b\phi)$ (lin $R^2$) | Bins used |
|---|---:|---:|---:|---:|---:|
| Proprietary | 588,543 → 588,334 → 588,172 | $Y=0.07264 \pm 0.00366$, $\gamma=0.3662 \pm 0.0083$ | 0.986 / 0.988 | $a=0.006771 \pm 0.000112$, $b=6650 \pm 305$ (0.923) | 29 |
| Client (non-proprietary) | 256,371 → 255,535 → 255,499 | $Y=0.1281 \pm 0.0154$, $\gamma=0.5082 \pm 0.0228$ | 0.959 / 0.951 | $a=0.01442 \pm 0.00061$, $b=682 \pm 54$ (0.993) | 23 |

**Conditioned on participation rate median ($\eta$ split; power-law only, all sides).**

| Group | $\eta < \eta_{\mathrm{median}}$ | $\eta \ge \eta_{\mathrm{median}}$ |
|---|---:|---:|
| Proprietary | $Y=0.08143 \pm 0.0077$, $\gamma=0.3800 \pm 0.0143$ | $Y=0.06475 \pm 0.00245$, $\gamma=0.3497 \pm 0.0063$ |
| Client (non-proprietary) | $Y=0.8106 \pm 0.150$, $\gamma=0.8383 \pm 0.0336$ | $Y=0.07291 \pm 0.00400$, $\gamma=0.3941 \pm 0.0095$ |

**Buy vs sell (power-law only).**

| Group | Buy $(Y,\gamma)$ | Sell $(Y,\gamma)$ |
|---|---:|---:|
| Proprietary | $Y=0.07218 \pm 0.00391$, $\gamma=0.3699 \pm 0.0090$ | $Y=0.07147 \pm 0.00498$, $\gamma=0.3589 \pm 0.0114$ |
| Client (non-proprietary) | $Y=0.1420 \pm 0.0221$, $\gamma=0.5311 \pm 0.0303$ | $Y=0.1073 \pm 0.0117$, $\gamma=0.4695 \pm 0.0201$ |

### Bivariate impact fit (power-law surface)

The script also fits the multiplicative form
$$
I/\sigma = C\,\eta^\delta\,\phi^\gamma, \qquad \phi \equiv Q/V.
$$

| Group | $C$ | $\delta$ | $\gamma$ | $R^2_{\log}$ / $R^2_{lin}$ |
|---|---:|---:|---:|---:|
| Proprietary (all) | 0.06343 ± 0.00323 | 0.3374 ± 0.0116 | 0.3458 ± 0.0091 | 0.949 / 0.964 |
| Client (non-proprietary, all) | 0.06147 ± 0.00647 | 0.3010 ± 0.0266 | 0.4022 ± 0.0185 | 0.860 / 0.832 |

### Nationality slices (member-level; caution on IT)

| Slice | Group | N after PR<1.0 | $(Y,\gamma)$ | Notes |
|---|---|---:|---:|---|
| Foreign | Proprietary | 585,930 | $Y=0.07096 \pm 0.00316$, $\gamma=0.3760 \pm 0.0076$ | close to full-sample fit |
| IT | Proprietary | 2,067 | $Y=0.01063 \pm 0.00585$, $\gamma=-0.0848 \pm 0.0946$ | unstable / low $R^2$ (tiny sample) |
| Foreign | Client (non-proprietary) | 231,617 | $Y=0.1026 \pm 0.0092$, $\gamma=0.4663 \pm 0.0172$ | stable |
| IT | Client (non-proprietary) | 23,862 | $Y=0.01399 \pm 0.00344$, $\gamma=0.1436 \pm 0.0876$ | low $R^2$; interpret cautiously |

### Plots (PNG)

**Overall impact curves (power-law + log overlay).**

| Proprietary | Client (non-proprietary) |
|---|---|
| ![Prop impact fit overall](../images/ftsemib/member_proprietary/png/power_law_fit_overall_member.png) | ![Client impact fit overall](../images/ftsemib/member_non_proprietary/png/power_law_fit_overall_member.png) |

**Buy vs sell fits.**

| Proprietary | Client (non-proprietary) |
|---|---|
| ![Prop impact fit by side](../images/ftsemib/member_proprietary/png/power_law_fit_overall_member_by_side.png) | ![Client impact fit by side](../images/ftsemib/member_non_proprietary/png/power_law_fit_overall_member_by_side.png) |

**Conditioned on participation rate ($\eta$).**

| Proprietary | Client (non-proprietary) |
|---|---|
| ![Prop fits by participation](../images/ftsemib/member_proprietary/png/power_law_fits_by_participation_rate.png) | ![Client fits by participation](../images/ftsemib/member_non_proprietary/png/power_law_fits_by_participation_rate.png) |

**Overlay: proprietary vs client.**

![Prop vs client overlay](../images/ftsemib/prop_vs_nonprop/png/power_law_prop_vs_nonprop.png)

**Impact surface heatmaps (Q/V vs $\eta$).**
...
**Bivariate fit surface (static PNG; interactive HTML exists).**

| Proprietary | Client (non-proprietary) |
|---|---|
| ![Prop bivariate fit surface](../images/ftsemib/member_proprietary/png/impact_surface_qv_duration_3d_surface_bivariate_fits_member_proprietary.png) | ![Client bivariate fit surface](../images/ftsemib/member_non_proprietary/png/impact_surface_qv_duration_3d_surface_bivariate_fits_member_non_proprietary.png) |

Interactive Plotly HTML files (open locally in a browser):

- Proprietary:
  - `../images/ftsemib/member_proprietary/html/impact_surface_qv_participation_3d_surface_member_proprietary.html`
  - `../images/ftsemib/member_proprietary/html/impact_surface_qv_duration_3d_surface_bivariate_fits_member_proprietary.html`
...
The script `scripts/metaorder_computation.py` applies an initial filter via `load_trades_filtered`:
...
In the default run (`scripts/metaorder_computation.py` WLS block), the fit uses `n_logbins=30`, `min_count=20`, mean impacts (not medians), and no control regressors. The helper `fit_power_law_logbins_wls_new` implements the weighting and optional controls described below.
...
Besides the univariate fit $I/\sigma \sim (Q/V)^\gamma$, `scripts/metaorder_computation.py` also implements a **bivariate power-law surface** that models the mean normalized impact jointly as a function of:
