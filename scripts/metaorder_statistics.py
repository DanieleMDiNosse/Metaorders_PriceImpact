...
- aggressive-member nationality share (`it` vs `foreign`)
...
2) Run:
    ...
    python scripts/metaorder_statistics.py
...
# `python scripts/metaorder_statistics.py` from the repo root.
...
# ---------------------------------------------------------------------
_CONFIG_ENV_VAR = "METAORDER_STATS_CONFIG"
...
if _config_override:
    ...
    _CONFIG_PATH = _REPO_ROOT / "config_ymls" / "metaorder_statistics.yml"
...
def save_plotly_figure(fig, *args, **kwargs):
    ...
    *args, **kwargs
    ...
    Notes
    -----
    Distribution figures are exported without top titles by design.
    """
...
def _normalize_member_nationality(value: object) -> Optional[str]:
    """Normalize member nationality labels to {'it', 'foreign'} when possible."""
...
def _load_members_nationality_map(path: Path) -> pd.Series:
    """Load mapping FIRM_ID_MODIF -> normalized nationality."""
    if not path.exists():
...
_MEMBER_NATIONALITY_MAP = _load_members_nationality_map(MEMBERS_NATIONALITY_PATH)


def _ensure_aggressive_member_nationality(trades: pd.DataFrame) -> pd.DataFrame:
    """Ensure `Aggressive Member Nationality` exists, mapping from `ID Member` when needed."""
    ...
    out[AGGRESSIVE_MEMBER_NATIONALITY_COL] = member_ids.map(_MEMBER_NATIONALITY_MAP)
    return out


def _infer_metaorder_nationality(labels: Sequence[object]) -> Tuple[Optional[str], bool]:
    """Infer one nationality label for a metaorder from its aggressive-side trades."""
...
def run_metaorder_dict_statistics(
    ...
    member_nationality:
        Optional aggressive-member nationality filter (`"it"` or `"foreign"`).
        When set, this must match the filter used to generate the input
        metaorder dictionary so index references stay aligned.
    ...
    Notes
    -----
    - Figures are generated with Plotly and exported as both HTML and PNG.
    - PNG export relies on Plotly static export support (typically `kaleido`).
    - The script also reports and plots aggressive-member nationality shares
      (`it` vs `foreign`) at the metaorder level.
    """
    ...
    for path in tqdm(parquet_paths, desc="ISINs"):
        ...
        for metas in metaorders_dict.values():
            for meta in metas:
                ...
                meta_nat, is_mixed = _infer_metaorder_nationality(
                    nationality_arr[meta_indices]
                )
                ...
                vols = buy_qty + sell_qty
                meta_volumes.append(vols)

                # Side inference: use the dominant aggressive-side quantity within the metaorder.
                # This is robust to occasional noisy trades in the opposite direction.
                if buy_qty > sell_qty:
                    n_buy_metaorders += 1
    ...
    known_side_metaorders = n_buy_metaorders + n_sell_metaorders
    print(
        "[Metaorder stats] Metaorder side counts (aggressive): "
        f"buy={n_buy_metaorders}, sell={n_sell_metaorders}, tie={n_tie_metaorders}, "
        f"known={known_side_metaorders}"
    )
    if counts_by_member:
        ...
        html_path, png_path = save_plotly_figure(
            fig_profile,
            stem=_with_member_nationality_tag("metaorders_per_member_all"),
            dirs=plot_dirs,
            write_html=True,
            write_png=True,
            strict_png=False,
        ...
        if png_path is None:
            print("[Metaorder stats][warn] Could not export Plotly PNG for metaorders_per_member_all.")
        else:
            print(f"[Metaorder stats] Saved PNG to {png_path}")

    known_nationality_metaorders = int(nationality_counts["it"] + nationality_counts["foreign"])
    print(
        "[Metaorder stats][Nationality] "
        f"proprietary_tag={METAORDER_STATS_PROPRIETARY_TAG}, "
        f"member_nationality_tag={MEMBER_NATIONALITY_TAG}, "
        f"it={nationality_counts['it']}, foreign={nationality_counts['foreign']}, "
        f"known={known_nationality_metaorders}, unknown={unknown_nationality_metaorders}, "
        f"mixed={mixed_nationality_metaorders}"
    )

    if known_nationality_metaorders > 0:
        pct_it = 100.0 * nationality_counts["it"] / known_nationality_metaorders
        pct_foreign = 100.0 * nationality_counts["foreign"] / known_nationality_metaorders
        print(
            "[Metaorder stats][Nationality] "
            f"Percentages (known only) -> it={pct_it:.2f}%, foreign={pct_foreign:.2f}%"
        )

        labels = ["it", "foreign"]
        counts_vals = [nationality_counts["it"], nationality_counts["foreign"]]
        pct_vals = [pct_it, pct_foreign]
        fig = go.Figure(
            data=[
                go.Bar(
                    x=labels,
                    y=pct_vals,
                    marker_color=[COLOR_PROPRIETARY, THEME_COLORWAY[2]],
                    text=[f"{pct:.1f}%<br>(n={cnt})" for pct, cnt in zip(pct_vals, counts_vals)],
        ...
        fig.update_layout(
            ...
            yaxis=dict(range=[0.0, max(100.0, max(pct_vals) + 12.0)]),
        )
        stem = _with_member_nationality_tag(f"nationality_share_{METAORDER_STATS_PROPRIETARY_TAG}")
        _, nationality_png_path = save_plotly_figure(
            fig,
            stem=stem,
            dirs=plot_dirs,
            write_html=True,
        ...
        if nationality_png_path is not None:
            print(f"[Metaorder stats][Nationality] Saved PNG to {nationality_png_path}")
        else:
            print("[Metaorder stats][warn] Could not export Plotly PNG for nationality share plot.")
    else:
        print(
            "[Metaorder stats][Nationality] Skipping nationality share plot: "
            "no metaorders with known aggressive-member nationality."
        )
    ...
    def _pdf_plot(
        ...
        if logx:
            x_min = float(np.nanmin(fit_data))
            x_max = float(np.nanmax(fit_data))
            if not np.isfinite(x_min) or not np.isfinite(x_max) or x_min <= 0.0 or x_max <= 0.0:
                tqdm.write(
                    f"Skipping plot {title}: log-x histogram requires strictly positive finite data; "
                    f"got x_min={x_min}, x_max={x_max}."
                )
                return
            if x_max <= x_min:
                # Degenerate support: create a tiny positive interval around the value so the density plot still renders.
                edges = np.array([x_min / 1.01, x_min * 1.01], dtype=float)
                edges[0] = max(edges[0], np.nextafter(0.0, 1.0))
            else:
                # Log-spaced bins avoid the visual distortion of linear bins on a log-x axis.
                edges = np.geomspace(x_min, x_max, int(bins) + 1)
            density, edges = np.histogram(fit_data, bins=edges, density=True)
            centers = np.sqrt(edges[:-1] * edges[1:])
        else:
            density, edges = np.histogram(fit_data, bins=bins, density=True)
            centers = 0.5 * (edges[:-1] + edges[1:])
        positive = np.isfinite(centers) & np.isfinite(density) & (density > 0)
        ...
        fig.add_trace(
            go.Scatter(
                ...
                marker=dict(size=5, color=COLOR_PROPRIETARY),
            )
        )
        median_value = float(np.median(fit_data))
        fig.add_vline(
            x=median_value,
            line=dict(color=THEME_COLORWAY[1], width=2, dash="dash"),
            opacity=0.9,
        )
        # add_vline shapes do not appear in legends; use a dummy trace so the
        # median value is visible in the legend for each density plot.
        fig.add_trace(
            go.Scatter(
                x=[None],
                y=[None],
            ...
            if fit_result is None:
                ...
                if np.isfinite(x_max) and x_max > fit_result.xmin:
                    ...
                    if np.any(valid):
                        ...
                        fig.add_vline(
                            x=float(fit_result.xmin),
                            line=dict(color=THEME_COLORWAY[2], width=1, dash="dash"),
                            opacity=0.7,
                        )
                        print(
        ...
        stem = _with_member_nationality_tag(Path(filename).stem)
        save_plotly_figure(
            fig,
            stem=stem,
            dirs=plot_dirs,
...
def main() -> None:
    ...
    >>> # python scripts/metaorder_statistics.py
    """
    log_path = OUTPUT_DIR / "logs" / _with_member_nationality_tag(
        f"metaorder_statistics_{METAORDER_STATS_LEVEL}_{METAORDER_STATS_PROPRIETARY_TAG}.log"
    )
    logger = setup_file_logger(Path(__file__).stem, log_path, mode="a")
    with PrintTee(logger):
        print("[Intro] Metaorder statistics run started...")
        print(
            "[Intro] Parameters — \n"
            f"  DATASET={DATASET_NAME}, LEVEL={METAORDER_STATS_LEVEL}, "
            f"PROPRIETARY={METAORDER_STATS_PROPRIETARY}, "
            f"PROPRIETARY_TAG={METAORDER_STATS_PROPRIETARY_TAG}, "
            f"MEMBER_NATIONALITY={MEMBER_NATIONALITY_TAG}, "
            f"TRADING_HOURS={TRADING_HOURS}, "
            f"RUN_METAORDER_DICT_STATS={RUN_METAORDER_DICT_STATS}, "
            f"METAORDER_POWERLAW_FIT_ENABLED={METAORDER_POWERLAW_FIT_ENABLED}, "
            f"METAORDER_POWERLAW_MIN_TAIL={METAORDER_POWERLAW_MIN_TAIL}, "
            f"METAORDER_POWERLAW_NUM_CANDIDATES={METAORDER_POWERLAW_NUM_CANDIDATES}, "
            f"METAORDER_POWERLAW_REFINE_WINDOW={METAORDER_POWERLAW_REFINE_WINDOW}"
        )
        print(
            "[Intro] Paths — \n"
            f"  PARQUET_PATH={PARQUET_DIR}\n"
            f"  OUTPUT_FILE_PATH={OUTPUT_DIR}\n"
            f"  IMG_OUTPUT_PATH={IMG_BASE_DIR}\n"
            f"  METAORDER_STATS_DICT_PATH={METAORDER_STATS_DICT_PATH}\n"
