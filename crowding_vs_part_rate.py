...
Activate the repository's main conda environment, then run:
    ...
    python crowding_vs_part_rate.py --dataset-name ftsemib

Or override paths:

    python crowding_vs_part_rate.py --prop-path out_files/ftsemib/...parquet \
...
_DEFAULT_CONFIG_PATH = _SCRIPT_DIR / "config_ymls" / "metaorder_statistics.yml"
...
def _try_git_hash() -> Optional[str]:
    ...
    try:
        out = subprocess.check_output(["git", "rev-parse", "HEAD"], cwd=str(_SCRIPT_DIR), text=True).strip()
    except Exception:
        return None
    return out or None
...
def _plotly_curve_date_ci(
    ...
    sp = np.argsort(x_prop)
    sc = np.argsort(x_client)

    fig = go.Figure()

    # Prop band + line
    fig.add_trace(go.Scatter(x=x_prop[sp], y=hi_prop[sp], mode="lines", line=dict(width=0), showlegend=False, hoverinfo="skip"))
    fig.add_trace(
        go.Scatter(
            x=x_prop[sp],
            y=lo_prop[sp],
    ...
    fig.add_trace(go.Scatter(x=x_prop[sp], y=y_prop[sp], mode="lines+markers", name="Prop", line=dict(color="rgb(31,119,180)")))

    # Client band + line
    fig.add_trace(go.Scatter(x=x_client[sc], y=hi_client[sc], mode="lines", line=dict(width=0), showlegend=False, hoverinfo="skip"))
    fig.add_trace(
        go.Scatter(
            x=x_client[sc],
            y=lo_client[sc],
    ...
    fig.add_trace(
        go.Scatter(x=x_client[sc], y=y_client[sc], mode="lines+markers", name="Client", line=dict(color="rgb(255,127,14)"))
    )

    fig.update_layout(title=title, height=520, width=900, legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
    fig.update_xaxes(title_text=x_label)
    fig.update_yaxes(title_text=y_label)
    fig.write_html(str(out_html), include_plotlyjs="cdn")
    fig.write_image(str(out_html.with_suffix(".png")), scale=2, width=900, height=520)
...
def _mpl_curve_date_ci(
    ...
    ax.fill_between(x_prop[sp], lo_prop[sp], hi_prop[sp], color="tab:blue", alpha=0.2, linewidth=0)
    ax.plot(x_prop[sp], y_prop[sp], color="tab:blue", marker="o", linewidth=1.5, label="Prop")

    ax.fill_between(x_client[sc], lo_client[sc], hi_client[sc], color="tab:orange", alpha=0.2, linewidth=0)
    ax.plot(x_client[sc], y_client[sc], color="tab:orange", marker="o", linewidth=1.5, label="Client")
...
def plotly_heatmap_align(
    ...
    fig.update_layout(title=title, xaxis_title="eta (bin center)", yaxis_title="Q/V (bin center)", height=600, width=850)
    out_html.parent.mkdir(parents=True, exist_ok=True)
    fig.write_html(str(out_html), include_plotlyjs="cdn")
            ...
            for metric, ylab in [
                ...
                if want_plotly:
                    _plotly_curve_date_ci(
                        ...
                        x_label="Participation rate (eta) bin center",
                        y_label=ylab,
                        title=title,
                        out_html=paths.img_dir / f"curve_{metric}_vs_eta_{imb_kind}.html",
                    )
                if want_mpl:
                    _mpl_curve_date_ci(
                        x_prop=prop_summary["eta_center"].to_numpy(dtype=float),
                        ...
                        y_label=ylab,
                        title=title,
                        out_png=paths.img_dir / f"curve_{metric}_vs_eta_{imb_kind}.png",
                    )
        ...
        if bool(args.run_2d) and want_plotly:
            for df_prepared, edges, group in [(prop_prep, edges_prop, "prop"), (client_prep, edges_client, "client")]:
                ...
                plotly_heatmap_align(
                    ...
                    out_html=paths.img_dir / f"heatmap_align_qv_eta_{group}_{imb_kind}.html",
                )
