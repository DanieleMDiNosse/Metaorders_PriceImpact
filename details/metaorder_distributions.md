...
The plots described here are produced by `run_metaorder_dict_statistics` in `metaorder_statistics.py` (enable `RUN_METAORDER_DICT_STATS=True`). They consume the metaorder indices saved by `metaorder_computation.py` in `out_files/{DATASET_NAME}/metaorders_dict_all_{LEVEL}_{PROPRIETARY_TAG}.pkl` together with the per-ISIN parquet tapes in `data/{DATASET_NAME}/`.

Metaorders are detected in `metaorder_computation.py` as contiguous same-sign runs for a single member or client, within one trading day and a single client ID. Runs are split when inter-trade gaps exceed `MAX_GAP=1h`, then filtered to keep at least `MIN_TRADES=5` trades and `SECONDS_FILTER=120` seconds of duration. Only trades in the continuous session 09:30:00-17:30:00 are used. Figures are written under `images/{DATASET_NAME}/{LEVEL}_{PROPRIETARY_TAG}/` (for example, `images/ftsemib/member_proprietary/` when `LEVEL="member"` and `PROPRIETARY=True`).
...
Figure: `images/{DATASET_NAME}/{LEVEL}_{PROPRIETARY_TAG}/metaorders_per_member_all.png`.
...
Figure: `images/{DATASET_NAME}/{LEVEL}_{PROPRIETARY_TAG}/metaorder_duration_all.png`.
...
Figure: `images/{DATASET_NAME}/{LEVEL}_{PROPRIETARY_TAG}/interarrival_all.png`.
...
Figure: `images/{DATASET_NAME}/{LEVEL}_{PROPRIETARY_TAG}/volumes_all.png`.
...
Figure: `images/{DATASET_NAME}/{LEVEL}_{PROPRIETARY_TAG}/q_over_v_all.png`.
...
Figure: `images/{DATASET_NAME}/{LEVEL}_{PROPRIETARY_TAG}/participation_rate_all.png`.
...
Each figure shows mean \(\pm 2\,\text{SE}\) versus \(\Delta\) for the three estimators. Outputs live under `images/{DATASET_NAME}/{LEVEL}_{PROPRIETARY_TAG}/signature_plots/`.
...
Figure: `images/{DATASET_NAME}/{LEVEL}_{PROPRIETARY_TAG}/normalized_impact_path_{LEVEL}_{PROPRIETARY_TAG}.png`.
...
- Overall fits: `images/{DATASET_NAME}/{LEVEL}_{PROPRIETARY_TAG}/power_law_fit_overall_{LEVEL}.png` (power-law plus logarithmic overlay).
- Fits conditioned on participation quantiles: `images/{DATASET_NAME}/{LEVEL}_{PROPRIETARY_TAG}/power_law_fits_by_participation_rate_{LEVEL}.png`.
